  ...
  methods: {
    mapInit () {
      MapLoader().then(AMap => {
        ...
        this.setNewCenter()
        this.initRoute(AMap)
        this.drawRoute([this.startPosition, this.driverPosition], AMap, 'real')
      }, e => {
    ...
    drawMarker (AMap) {
      ...
      endMarker.setLabel({
        ...
        content: `<div class='info'>收</div>`, // 设置文本标注内容
      ...
      driving.search(new AMap.LngLat(this.driverPosition[0], this.driverPosition[1]), new AMap.LngLat(this.endPosition[0], this.endPosition[1]), (status, result) => {
        if (status === 'complete') {
          if (result.routes && result.routes.length) {
            ...
            this.drawRoute(result.routes[0], AMap, 'project')
          }
        } else {
